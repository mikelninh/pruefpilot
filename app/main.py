...
app = FastAPI(
    title="PrüfPilot Document AI", version="5.5.0",
    description="Reusable public-sector case engine with grounded RAG, bounded agents, source provenance, durable production operations and human approval.",
...
def original_document(document_id: str, request: Request) -> Response:
    tenant_id = _tenant_id(request)
    blob = store.get_blob(document_id, tenant_id=tenant_id)
    if blob is None:
        raise HTTPException(status_code=404, detail="Document not found")
    digest = hashlib.sha256(blob).hexdigest()
    _append_audit(request, "document.original_viewed", {"document_id": document_id, "sha256": digest})
    return Response(
        content=blob,
        ...
        headers={
            ...
            "x-source-integrity": "sha256-recomputed-on-read",
        },
    )
...
def product_brief() -> dict:
    return {
        ...
        "production_v1": [
            ...
            "source provenance + SHA-256 integrity", "original-source retrieval with recomputed hash",
        ...
        "production_next": ["DMS/funding-system adapters", "verified issuer/signature adapters", "measured live SLOs", "qualified reviewer + external security validation"],
        "external_next": ["DMS/funding-system adapters", "verified issuer/signature adapters", "measured live SLOs", "qualified reviewer + external security validation"],
    }
